...
function main(parsed_args)
    ...
    @variable(m, X[1:dimension,1:dimension], SDP)
    @constraint(m, diag(X) .== ones(dimension, 1))
    @objective(m, Min, trace(Cost_Matrix*X))
        
    status = solve(m)
        
    SDPobjval = getobjectivevalue(m);
    Xval = getvalue(X);
        
    t2 = time_ns();
    total_time = (t2 - t1)/1.0e9;    
    ...
    for cut_trials = 1:rounding_trials
        r = rand(Normal(), 1, dimension);
        r = r/sqrt(dot(r, r));
        cut = zeros(Float64, dimension, 1);
        for cut_iter = 1:dimension
            cut[cut_iter, 1] =  sign(dot(r, Xval[:, cut_iter]));
        end
